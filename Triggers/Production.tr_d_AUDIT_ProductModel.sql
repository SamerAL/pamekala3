...
GO
CREATE TRIGGER [Production].[tr_d_AUDIT_ProductModel]
...
-- Description:  DELETE TRIGGER for Table:  [Production].[ProductModel]
...
BEGIN
	...
	SELECT
		@AUDIT_LOG_TRANSACTION_ID,
		convert(nvarchar(1500), IsNull('[ProductModelID]='+CONVERT(nvarchar(4000), OLD.[ProductModelID], 0), '[ProductModelID] Is Null')),
		'ProductModelID',
		CONVERT(nvarchar(4000), OLD.[ProductModelID], 0),
		'A'
		,  CONVERT(nvarchar(500), CONVERT(nvarchar(4000), OLD.[ProductModelID], 0))
	FROM deleted OLD
	WHERE
...
END
GO
EXEC sp_settriggerorder @triggername= '[Production].[tr_d_AUDIT_ProductModel]', @order='Last', @stmttype='DELETE'
GO
