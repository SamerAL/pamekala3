...
GO
CREATE TRIGGER [Production].[tr_u_AUDIT_Culture]
...
-- Description:  UPDATE TRIGGER for Table:  [Production].[Culture]
...
BEGIN
	...
	BEGIN
		...
		SELECT
			@AUDIT_LOG_TRANSACTION_ID,
		    convert(nvarchar(1500), IsNull('[CultureID]=N'''+replace(CONVERT(nvarchar(4000), IsNull(OLD.[CultureID], NEW.[CultureID]), 0),'''' ,'''''')+'''', '[CultureID] Is Null')),
		    'CultureID',
			CONVERT(nvarchar(4000), OLD.[CultureID], 0),
			CONVERT(nvarchar(4000), NEW.[CultureID], 0),
			'A'
			, IsNULL( CONVERT(nvarchar(500), CONVERT(nvarchar(4000), OLD.[CultureID], 0)), CONVERT(nvarchar(500), CONVERT(nvarchar(4000), NEW.[CultureID], 0)))
			
		FROM deleted OLD Full Outer Join inserted NEW On
			(CONVERT(nvarchar(4000), NEW.[CultureID], 0)=CONVERT(nvarchar(4000), OLD.[CultureID], 0) or (NEW.[CultureID] Is Null and OLD.[CultureID] Is Null))
			WHERE (
...
End
GO
EXEC sp_settriggerorder @triggername= '[Production].[tr_u_AUDIT_Culture]', @order='Last', @stmttype='UPDATE'
GO
