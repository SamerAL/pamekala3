...
GO
CREATE TRIGGER [Sales].[tr_d_AUDIT_SalesTaxRate]
...
-- Description:  DELETE TRIGGER for Table:  [Sales].[SalesTaxRate]
...
BEGIN
	...
	SELECT
		@AUDIT_LOG_TRANSACTION_ID,
		convert(nvarchar(1500), IsNull('[SalesTaxRateID]='+CONVERT(nvarchar(4000), OLD.[SalesTaxRateID], 0), '[SalesTaxRateID] Is Null')),
		'SalesTaxRateID',
		CONVERT(nvarchar(4000), OLD.[SalesTaxRateID], 0),
		'A'
		,  CONVERT(nvarchar(500), CONVERT(nvarchar(4000), OLD.[SalesTaxRateID], 0))
	FROM deleted OLD
	WHERE
...
END
GO
EXEC sp_settriggerorder @triggername= '[Sales].[tr_d_AUDIT_SalesTaxRate]', @order='Last', @stmttype='DELETE'
GO
