...
GO
CREATE TRIGGER [Person].[tr_u_AUDIT_Password]
ON [Person].[Password]
FOR UPDATE
...
-- Description:  UPDATE TRIGGER for Table:  [Person].[Password]
...
BEGIN
	...
	--Set @TABLE_NAME = '[Person].[Password]'
	...
	BEGIN
		...
		SELECT
		    ...
		    convert(nvarchar(1500), IsNull('[BusinessEntityID]='+CONVERT(nvarchar(4000), IsNull(OLD.[BusinessEntityID], NEW.[BusinessEntityID]), 0), '[BusinessEntityID] Is Null')),
		    'PasswordHash',
			CONVERT(nvarchar(4000), OLD.[PasswordHash], 0),
			CONVERT(nvarchar(4000), NEW.[PasswordHash], 0),
			'A'
			, IsNULL( CONVERT(nvarchar(500), CONVERT(nvarchar(4000), OLD.[BusinessEntityID], 0)), CONVERT(nvarchar(500), CONVERT(nvarchar(4000), NEW.[BusinessEntityID], 0)))
		...
		FROM deleted OLD Inner Join inserted NEW On 
			...
			where (
				...
				(
					NEW.[PasswordHash] Is Null And
					OLD.[PasswordHash] Is Not Null
				) Or
				(
					NEW.[PasswordHash] Is Not Null And
					OLD.[PasswordHash] Is Null
				)
				)
	...
	BEGIN
		...
		SELECT
		    ...
		    convert(nvarchar(1500), IsNull('[BusinessEntityID]='+CONVERT(nvarchar(4000), IsNull(OLD.[BusinessEntityID], NEW.[BusinessEntityID]), 0), '[BusinessEntityID] Is Null')),
		    'PasswordSalt',
			CONVERT(nvarchar(4000), OLD.[PasswordSalt], 0),
			CONVERT(nvarchar(4000), NEW.[PasswordSalt], 0),
			'A'
			, IsNULL( CONVERT(nvarchar(500), CONVERT(nvarchar(4000), OLD.[BusinessEntityID], 0)), CONVERT(nvarchar(500), CONVERT(nvarchar(4000), NEW.[BusinessEntityID], 0)))
		...
		FROM deleted OLD Inner Join inserted NEW On 
			...
			where (
				...
				(
					NEW.[PasswordSalt] Is Null And
					OLD.[PasswordSalt] Is Not Null
				) Or
				(
					NEW.[PasswordSalt] Is Not Null And
					OLD.[PasswordSalt] Is Null
				)
				)
...
End
GO
EXEC sp_settriggerorder @triggername= '[Person].[tr_u_AUDIT_Password]', @order='Last', @stmttype='UPDATE'
GO
