...
GO
CREATE TRIGGER [Production].[tr_i_AUDIT_ProductReview]
...
-- Description:  INSERT TRIGGER for Table:  [Production].[ProductReview]
...
BEGIN
	...
	SELECT
		@AUDIT_LOG_TRANSACTION_ID,
		convert(nvarchar(1500), IsNull('[ProductReviewID]='+CONVERT(nvarchar(4000), NEW.[ProductReviewID], 0), '[ProductReviewID] Is Null')),
		'ProductReviewID',
		CONVERT(nvarchar(4000), NEW.[ProductReviewID], 0),
		'A'
		, CONVERT(nvarchar(500), CONVERT(nvarchar(4000), NEW.[ProductReviewID], 0))
	FROM inserted NEW
	WHERE NEW.[ProductReviewID] Is Not Null
	...
	SELECT
		...
		, CONVERT(nvarchar(500), CONVERT(nvarchar(4000), NEW.[ProductReviewID], 0))
	FROM inserted NEW
	WHERE NEW.[ReviewerName] Is Not Null
	...
	SELECT
		...
		, CONVERT(nvarchar(500), CONVERT(nvarchar(4000), NEW.[ProductReviewID], 0))
	FROM inserted NEW
	WHERE NEW.[ReviewDate] Is Not Null
...
End
GO
EXEC sp_settriggerorder @triggername= '[Production].[tr_i_AUDIT_ProductReview]', @order='Last', @stmttype='INSERT'
GO
