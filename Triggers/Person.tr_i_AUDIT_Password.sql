...
GO
CREATE TRIGGER [Person].[tr_i_AUDIT_Password]
ON [Person].[Password]
FOR INSERT
...
-- Description:  INSERT TRIGGER for Table:  [Person].[Password]
...
BEGIN
	...
	SELECT
		...
		, CONVERT(nvarchar(500), CONVERT(nvarchar(4000), NEW.[BusinessEntityID], 0))
	FROM inserted NEW
	WHERE NEW.[PasswordHash] Is Not Null
	...
	SELECT
		...
		, CONVERT(nvarchar(500), CONVERT(nvarchar(4000), NEW.[BusinessEntityID], 0))
	FROM inserted NEW
	WHERE NEW.[PasswordSalt] Is Not Null
...
End
GO
EXEC sp_settriggerorder @triggername= '[Person].[tr_i_AUDIT_Password]', @order='Last', @stmttype='INSERT'
GO
