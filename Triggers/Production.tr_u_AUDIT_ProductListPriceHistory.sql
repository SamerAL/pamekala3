...
GO
CREATE TRIGGER [Production].[tr_u_AUDIT_ProductListPriceHistory]
...
-- Description:  UPDATE TRIGGER for Table:  [Production].[ProductListPriceHistory]
...
BEGIN
	...
	BEGIN
		...
		SELECT
		    ...
		    convert(nvarchar(1500), IsNull('[ProductID]='+CONVERT(nvarchar(4000), IsNull(OLD.[ProductID], NEW.[ProductID]), 0), '[ProductID] Is Null')+' AND '+IsNull('[StartDate]=N'''+replace(CONVERT(nvarchar(4000), IsNull(OLD.[StartDate], NEW.[StartDate]), 121),'''' ,'''''')+'''', '[StartDate] Is Null')),
		    'ListPrice',
			CONVERT(nvarchar(4000), OLD.[ListPrice], 2),
		...
		FROM deleted OLD Inner Join inserted NEW On 
			...
			where (
				...
				(
					NEW.[ListPrice] Is Null And
					OLD.[ListPrice] Is Not Null
				) Or
				(
					NEW.[ListPrice] Is Not Null And
					OLD.[ListPrice] Is Null
				)
				)
...
End
GO
EXEC sp_settriggerorder @triggername= '[Production].[tr_u_AUDIT_ProductListPriceHistory]', @order='Last', @stmttype='UPDATE'
GO
