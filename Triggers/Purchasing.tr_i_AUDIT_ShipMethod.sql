...
GO
CREATE TRIGGER [Purchasing].[tr_i_AUDIT_ShipMethod]
...
-- Description:  INSERT TRIGGER for Table:  [Purchasing].[ShipMethod]
...
BEGIN
	...
	SELECT
		@AUDIT_LOG_TRANSACTION_ID,
		convert(nvarchar(1500), IsNull('[ShipMethodID]='+CONVERT(nvarchar(4000), NEW.[ShipMethodID], 0), '[ShipMethodID] Is Null')),
		'ShipMethodID',
		CONVERT(nvarchar(4000), NEW.[ShipMethodID], 0),
		'A'
		, CONVERT(nvarchar(500), CONVERT(nvarchar(4000), NEW.[ShipMethodID], 0))
	FROM inserted NEW
	...
	SELECT
		...
		, CONVERT(nvarchar(500), CONVERT(nvarchar(4000), NEW.[ShipMethodID], 0))
	FROM inserted NEW
	WHERE NEW.[ShipBase] Is Not Null
	...
	SELECT
		...
		, CONVERT(nvarchar(500), CONVERT(nvarchar(4000), NEW.[ShipMethodID], 0))
	FROM inserted NEW
	WHERE NEW.[ShipRate] Is Not Null
...
End
GO
EXEC sp_settriggerorder @triggername= '[Purchasing].[tr_i_AUDIT_ShipMethod]', @order='Last', @stmttype='INSERT'
GO
