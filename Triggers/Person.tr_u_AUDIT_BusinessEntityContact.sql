...
GO
CREATE TRIGGER [Person].[tr_u_AUDIT_BusinessEntityContact]
...
-- Description:  UPDATE TRIGGER for Table:  [Person].[BusinessEntityContact]
...
BEGIN
	...
	BEGIN
		...
		SELECT
			@AUDIT_LOG_TRANSACTION_ID,
		    convert(nvarchar(1500), IsNull('[BusinessEntityID]='+CONVERT(nvarchar(4000), IsNull(OLD.[BusinessEntityID], NEW.[BusinessEntityID]), 0), '[BusinessEntityID] Is Null')+' AND '+IsNull('[PersonID]='+CONVERT(nvarchar(4000), IsNull(OLD.[PersonID], NEW.[PersonID]), 0), '[PersonID] Is Null')+' AND '+IsNull('[ContactTypeID]='+CONVERT(nvarchar(4000), IsNull(OLD.[ContactTypeID], NEW.[ContactTypeID]), 0), '[ContactTypeID] Is Null')),
		    'BusinessEntityID',
			CONVERT(nvarchar(4000), OLD.[BusinessEntityID], 0),
			CONVERT(nvarchar(4000), NEW.[BusinessEntityID], 0),
			'A'
			, IsNULL( CONVERT(nvarchar(500), CONVERT(nvarchar(4000), OLD.[BusinessEntityID], 0)), CONVERT(nvarchar(500), CONVERT(nvarchar(4000), NEW.[BusinessEntityID], 0))), IsNULL( CONVERT(nvarchar(500), CONVERT(nvarchar(4000), OLD.[PersonID], 0)), CONVERT(nvarchar(500), CONVERT(nvarchar(4000), NEW.[PersonID], 0))), IsNULL( CONVERT(nvarchar(500), CONVERT(nvarchar(4000), OLD.[ContactTypeID], 0)), CONVERT(nvarchar(500), CONVERT(nvarchar(4000), NEW.[ContactTypeID], 0)))
			
		FROM deleted OLD Full Outer Join inserted NEW On
			(CONVERT(nvarchar(4000), NEW.[BusinessEntityID], 0)=CONVERT(nvarchar(4000), OLD.[BusinessEntityID], 0) or (NEW.[BusinessEntityID] Is Null and OLD.[BusinessEntityID] Is Null)) AND (CONVERT(nvarchar(4000), NEW.[PersonID], 0)=CONVERT(nvarchar(4000), OLD.[PersonID], 0) or (NEW.[PersonID] Is Null and OLD.[PersonID] Is Null)) AND (CONVERT(nvarchar(4000), NEW.[ContactTypeID], 0)=CONVERT(nvarchar(4000), OLD.[ContactTypeID], 0) or (NEW.[ContactTypeID] Is Null and OLD.[ContactTypeID] Is Null))
			WHERE (
...
End
GO
EXEC sp_settriggerorder @triggername= '[Person].[tr_u_AUDIT_BusinessEntityContact]', @order='Last', @stmttype='UPDATE'
GO
