...
GO
CREATE TRIGGER [Sales].[tr_i_AUDIT_SalesTerritory]
...
-- Description:  INSERT TRIGGER for Table:  [Sales].[SalesTerritory]
...
BEGIN
	...
	SELECT
		@AUDIT_LOG_TRANSACTION_ID,
		convert(nvarchar(1500), IsNull('[TerritoryID]='+CONVERT(nvarchar(4000), NEW.[TerritoryID], 0), '[TerritoryID] Is Null')),
		'TerritoryID',
		CONVERT(nvarchar(4000), NEW.[TerritoryID], 0),
		'A'
		, CONVERT(nvarchar(500), CONVERT(nvarchar(4000), NEW.[TerritoryID], 0))
	FROM inserted NEW
	...
	SELECT
		...
		, CONVERT(nvarchar(500), CONVERT(nvarchar(4000), NEW.[TerritoryID], 0))
	FROM inserted NEW
	WHERE NEW.[CostLastYear] Is Not Null
...
End
GO
EXEC sp_settriggerorder @triggername= '[Sales].[tr_i_AUDIT_SalesTerritory]', @order='Last', @stmttype='INSERT'
GO
