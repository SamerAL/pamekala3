...
GO
CREATE TRIGGER [Sales].[tr_i_AUDIT_SpecialOffer]
...
-- Description:  INSERT TRIGGER for Table:  [Sales].[SpecialOffer]
...
BEGIN
	...
	SELECT
		@AUDIT_LOG_TRANSACTION_ID,
		convert(nvarchar(1500), IsNull('[SpecialOfferID]='+CONVERT(nvarchar(4000), NEW.[SpecialOfferID], 0), '[SpecialOfferID] Is Null')),
		'SpecialOfferID',
		CONVERT(nvarchar(4000), NEW.[SpecialOfferID], 0),
		'A'
		, CONVERT(nvarchar(500), CONVERT(nvarchar(4000), NEW.[SpecialOfferID], 0))
	FROM inserted NEW
	...
	SELECT
		...
		, CONVERT(nvarchar(500), CONVERT(nvarchar(4000), NEW.[SpecialOfferID], 0))
	FROM inserted NEW
	WHERE NEW.[DiscountPct] Is Not Null
...
End
GO
EXEC sp_settriggerorder @triggername= '[Sales].[tr_i_AUDIT_SpecialOffer]', @order='Last', @stmttype='INSERT'
GO
